...
if (@ARGV != 2) {
        print "Usage: ./single_firstclass2imap.pl <instance> <destination_email_address>\n\n";
        ...
        print "Example: ./single_firstclass2imap.pl 1 fc_user\n\n";
...
# this query is helpful during testing ... it limits the migration to a specific account
       my($sth) = $dbh->prepare("SELECT switched, fromuser, fromfolder, touser, topassword, recursive, migrated, migrating FROM usermap WHERE fromuser = '$fc_user'");
